...
/// @brief headerrs templates for the ppm header record and ppm file names
...
static void dump_ppm(const void *p, int size, unsigned int tag, struct timespec *time)
{
    ...
    snprintf(&ppm_dumpname[11], 10, "%05d", tag);
    strncat(&ppm_dumpname[16], ".ppm", 5);
    dumpfd = open(ppm_dumpname, O_WRONLY | O_NONBLOCK | O_CREAT, 00666);
    if (dumpfd<0){
        char err_mess[1024]="";
        sprintf(err_mess,"Error un dump file opening: %s\n",ppm_dumpname);
        errno_exit(err_mess);
    }
    ...
    if (written<0){
        char err_mess[1024]="";
        sprintf(err_mess,"Error on writing the dump fileg: %s\n",ppm_dumpname);
        errno_exit(err_mess);
    }
...
int frame_edges_detection(void)
{
    ...
    for(int frame_cnt= frames_num; frame_cnt>0; frame_cnt--){
        ...
        //MST <<< dump_pgm(edged_data_ptr, (FRAME_SIZE/2), edge_framecnt, &time_now);
